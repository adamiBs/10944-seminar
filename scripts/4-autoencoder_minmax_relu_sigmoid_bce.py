...
plt.title('Autoencoder Training Loss (MinMax, ReLU, Sigmoid Output, BCE)')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_minmax_relu_sigmoid_bce_loss.png', dpi=300)
plt.close()
...
plt.title('Latent Space: MinMax + ReLU + Sigmoid + BCE', fontsize=14)
plt.xlabel('Latent Dimension 1', fontsize=12)
plt.ylabel('Latent Dimension 2', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_minmax_relu_sigmoid_bce.png', dpi=300)
plt.close()
...
metrics_path = '/workspaces/10944-seminar/images/4-autoencoder_minmax_relu_sigmoid_bce_metrics.txt'
...
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_minmax_relu_sigmoid_bce_metrics.png', dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_minmax_relu_sigmoid_bce_samples.png', dpi=300)
plt.close()
...
1. MinMax normalization with BCE loss is theoretically ideal
   - MinMax scales data to [0,1], which can be interpreted as probabilities
   - Binary Cross-Entropy treats each pixel as an independent Bernoulli trial
   - This is a perfect match in terms of statistical interpretation
   
2. ReLU activation with MinMax normalized data works very well
   - All inputs are positive, so ReLU passes all values unchanged
   - No "dying ReLU" problem since there are no negative inputs
   - Creates stable and effective feature extraction
   
3. Sigmoid output is perfectly matched with BCE loss
   - Sigmoid outputs can be interpreted as probabilities in [0,1]
   - BCE optimizes these probabilistic predictions directly
   - This combination often produces sharper, more detailed reconstructions
   
4. This configuration represents the most theoretically sound combination
   - All components (normalization, activation, output, loss) are aligned
   - Should result in faster convergence and better reconstructions
   - May create more meaningful and structured latent space representations
   - Particularly well-suited for image data where each pixel can be seen as a probability
