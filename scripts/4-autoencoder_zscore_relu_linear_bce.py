...
plt.title('Autoencoder Training Loss (Z-score, ReLU, Linear Output, BCE Loss)')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_bce_loss.png', dpi=300)
plt.close()
...
plt.title('Latent Space: Z-score + ReLU + Linear + BCE', fontsize=14)
plt.xlabel('Latent Dimension 1', fontsize=12)
plt.ylabel('Latent Dimension 2', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_bce.png', dpi=300)
plt.close()
...
metrics_path = '/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_bce_metrics.txt'
...
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_bce_metrics.png', dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_bce_samples.png', dpi=300)
plt.close()
...
1. Z-score normalization centers the data around mean=0 with std=1
   - This creates a potential mismatch with binary crossentropy which works best with [0,1] range data
   
2. ReLU activation with Z-score normalized data may cause issues
   - Since ~50% of Z-score normalized values are negative, many neurons may be inactive
   - This can limit the expressiveness of the network and lead to "dying ReLU" problem
   
3. Linear output activation with BCE loss creates a unique combination
   - Linear output can produce any value, while BCE loss works best with [0,1] range
   - This mismatch may affect convergence and reconstruction quality
   - The network may struggle to learn meaningful representations with this combination
   
4. Binary crossentropy is typically better suited for sigmoid outputs
   - When used with linear outputs and Z-score normalized data, BCE may lead to
     unstable training or poor reconstruction quality
   - This unusual pairing may be useful for experimental comparison only
