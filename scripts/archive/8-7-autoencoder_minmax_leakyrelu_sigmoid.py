...
plt.title('Autoencoder Training Loss (MinMax, LeakyReLU 0.3, Sigmoid Output)')
plt.xlabel('Epochs')
plt.ylabel('Loss (MSE)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/8-7-autoencoder_minmax_leakyrelu_sigmoid_loss.png', dpi=300)
plt.close()
...
plt.title('Autoencoder Latent Space\nMinMax + LeakyReLU(0.3) + Sigmoid Output', fontsize=14)
plt.xlabel('Latent Dimension 1', fontsize=12)
plt.ylabel('Latent Dimension 2', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/8-7-autoencoder_minmax_leakyrelu_sigmoid.png', dpi=300)
plt.close()
...
1. MinMax normalization scales the data to the range [0,1], which aligns perfectly
   with the sigmoid output activation range [0,1], creating a good match between
   input and output distributions.
   
2. This matching of ranges typically results in lower reconstruction error compared
   to using Z-score normalization with sigmoid output.
   
3. LeakyReLU(0.3) with a higher alpha value allows more gradient flow for negative inputs,
   which may allow the model to learn more expressive features even from near-zero
   MinMax-normalized inputs.
   
4. The sigmoid output could introduce vanishing gradient issues for values close to
   0 or 1, but this effect should be minimal with MinMax-normalized data that's
   already properly scaled.
   
5. This configuration should typically perform well with the digits dataset as there's
   no range mismatch that would create learning challenges for the network.
"""