...
plt.title('Autoencoder Training Loss (Z-score, ReLU, Sigmoid Output)')
plt.xlabel('Epochs')
plt.ylabel('Loss (MSE)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-7-autoencoder_zscore_relu_sigmoid_loss.png', dpi=300)
plt.close()
...
plt.title('Autoencoder Latent Space\nZ-score + ReLU + Sigmoid Output', fontsize=14)
plt.xlabel('Latent Dimension 1', fontsize=12)
plt.ylabel('Latent Dimension 2', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-7-autoencoder_zscore_relu_sigmoid.png', dpi=300)
plt.close()
...
1. Z-score normalization centers the data with mean=0 and std=1, but the sigmoid output
   function compresses values to the range [0,1], which creates a mismatch.
   
2. The combination may lead to higher reconstruction error since the sigmoid output
   cannot accurately represent the full range of normalized data. The network must learn
   to internally rescale the data before the output layer.
   
3. Standard ReLU activation could potentially suffer from "dying ReLU" problem where
   neurons become inactive, especially with Z-score normalized input where
   approximately 50% of the values are negative.
   
4. The sigmoid output may cause saturation in the gradients during early training stages,
   potentially slowing down learning in the deeper layers of the network.
   
5. This configuration might create a bottleneck at the output layer where Z-score data
   with potentially large positive and negative values must be compressed into [0,1].
"""