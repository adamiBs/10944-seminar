...
plt.title('Autoencoder Training Loss (Z-score, ReLU, Linear Output, MSE Loss)')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_mse_loss.png', dpi=300)
plt.close()
...
plt.title('Latent Space: Z-score + ReLU + Linear + MSE', fontsize=14)
plt.xlabel('Latent Dimension 1', fontsize=12)
plt.ylabel('Latent Dimension 2', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_mse.png', dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig('/workspaces/10944-seminar/images/4-autoencoder_zscore_relu_linear_mse_samples.png', dpi=300)
plt.close()
...
1. Z-score normalization centers the data around mean=0 with std=1
   - This normalization is generally a good match with linear output since the ranges align
   
2. ReLU activation may suffer from "dying ReLU" problem with Z-score normalized data
   - Since approximately half of the Z-score normalized values are negative, ReLU will zero these out
   - This could lead to less effective feature learning in the network
   
3. Linear output activation allows for unbounded outputs
   - This works well with Z-score normalization since the data is not bounded
   - The model can predict both positive and negative values as needed
